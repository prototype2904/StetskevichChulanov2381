    ...
    variantSquares()
{
    ...
    buttonModeNormal->setGeometry(QRect(872,290, 250,120));
    buttonMode50-> setGeometry(QRect(872,410, 250, 120));
    buttonMode55-> setGeometry(QRect(872,550, 250, 120));
    undoButton->setGeometry(QRect(872,290, 250,120));
    saveGameButton -> setGeometry(QRect(872,410, 250, 120));
    loadGameButton -> setGeometry(QRect(872,610, 250, 60));
    buttonStartGame -> setGeometry(QRect(872,410, 250, 120));
    buttonPause -> setGeometry(QRect(872, 150, 100,100));
    buttonBack -> setGeometry(QRect(872, 150, 100, 100));
    labelCheckMate->setGeometry(QRect(350, 50, 180, 40));


    lcdSeconds->setGeometry(QRect(942,160, 250,50));
    lcdMinutes->setGeometry(QRect(872,160, 250,50));

    lineEditInputLoadFile ->setGeometry(QRect(872,550, 250,60));
...
//    connect(this, SLOT(nextSec(int)), ui->timer, SIGNAL(display(int)));
    ...
    qDebug() << path;
    files.push_back(path+"/image/figure_black/");
    files.push_back(path+"/image/figure_white/");
    files.push_back(path+"/image/figure_blue/");
    for(int i = 0; i < 3; i++)
    {
...
void Board::saveGame()
{
    if(Game::moves.size() > 0)
    {
        QTime time;
        time.currentTime();
        qDebug() << time.currentTime().toString("hh-mm-ss-zzz");
        std::fstream file;
        QString path = QApplication::applicationDirPath();
        const string str = path.toStdString() + "\\" + QTime::currentTime().toString("hh-mm-ss-zzz").toStdString() +".txt";
...
void Board::paintEvent(QPaintEvent *pe)
{
    ...
    if(Game::mode != -1)
    {
        ...
        QPainter p(this);
        QRadialGradient gradient(418, 379, 340, 418,379);
        ...
        else if(Game::movePlayer == 1)
        {
            ...
            //draw board!!!
            QString path = QCoreApplication::applicationDirPath();
            QImage im(path+"/image/board.png");

            p.drawImage(rec,QImage(path+"/image/wallpaper.png"));


            QBrush brush(gradient);
            p.setPen(QPen(QColor::fromRgbF(0,0,0,0)));
            p.setBrush(brush);
            p.drawEllipse(418- 320, 379-320, 320*2, 320*2);
